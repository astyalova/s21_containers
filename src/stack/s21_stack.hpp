...
template <class T> stack<T>::stack(stack &&s) = default;
...
template <class T> bool stack<T>::empty() { return stack_.empty(); }
...
template <class T> void stack<T>::pop() { stack_.pop_back(); }
