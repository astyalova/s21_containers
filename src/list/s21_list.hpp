...
template <class T> list<T> &list<T>::operator=(list &&l) {
  if (this != &l) {
    clear();
...
template <class T> typename list<T>::const_reference list<T>::front() {
  return head_->data;
}

template <class T> typename list<T>::const_reference list<T>::back() {
  return tail_->data;
}
...
template <typename T> bool list<T>::empty() { return node_size_ == 0; }
...
template <class T> void list<T>::splice(iterator pos, list &other) {
  if (pos != end()) {
