...
TEST(Operator, Assignment_operator_list) {
    ...
    s21::list<std:: string> list = {"fdsggfdgvfg","ggewfewrewrf", "rtregfedfgeg", "", "43949324"};
...
TEST(List_Capacity, Max_Size) {
    ...
    size_t expected_max_size_double = std::numeric_limits<size_t>::max() / sizeof(double);
    EXPECT_EQ(list2.max_size(), expected_max_size_double);

    s21::list<char> list3;
    size_t expected_max_size_char = std::numeric_limits<size_t>::max() / sizeof(char);
    EXPECT_EQ(list3.max_size(), expected_max_size_char);

    s21::list<std::string> list4;
    size_t expected_max_size_str = std::numeric_limits<size_t>::max() / sizeof(std::string);
